...
class inspectionForm(ModelForm):
    ...
    class Meta:
        model = inspection
        fields = ['claimID', 'vin', 'make', 'model', 'year',
                  'colour', 'status', 'scheduledDate', 'scheduledTime']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user.is_staff == False:
            self.fields['claimID'].queryset = claim.objects.filter(
                warrantyID__customerID__email=user.email)
            print("not a staff")
