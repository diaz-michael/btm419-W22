...
def inventory(request):
    ...
    if is_valid_queryparam(values, 'product', product_query) and product_query != "Order includes...":
        qs = qs.filter(id__in=qs.filter(order__productID__name__icontains=product_query))


    productcounts = qs.values('order__productID__name').annotate(sum=Sum('order__quantity')).order_by('order__productID__name')
    summarytable = qs.aggregate(Avg('totalsum'),Max('totalsum'), Min('totalsum'), Count('id'))
    ...
    # print(sums)
    fig, ax = plt.subplots()
    ax.pie(sums, labels=labels, autopct=make_autopct(sums), pctdistance=0.8, startangle=90)
    ax.axis('equal')
